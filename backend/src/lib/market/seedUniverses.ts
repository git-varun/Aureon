/** Port of app/modules/market/seed_universes.py — curated, live-verified
 * constituent lists for the 5 index-based tracked equity universes (Phase
 * D). Byte-for-byte copy of Python's lists (including its documented
 * drop/rename notes below) — crypto has no static list here, see
 * seedTrackedUniverses.ts's seedCryptoTop100 (CoinGecko top-100 discovery). */

// TATAMOTORS.NS -> TMPV.NS: live-tested empty even at 5y history — Tata
// Motors demerged into passenger/commercial vehicle entities; TMPV.NS (Tata
// Motors Passenger Vehicles) is the real live successor. ZOMATO.NS ->
// ETERNAL.NS: live-tested empty — Zomato's 2024 corporate rebrand to
// Eternal Ltd.
const INDIA_NIFTY100 = [
...
// MMC (Marsh & McLennan) dropped — live-tested empty at 5d/1mo history
// despite being a real, currently-listed S&P 100 company; not a
// symbol-format issue — left out rather than guessed at, per no-fake-data.
const US_SP100 = [
...
// .T = Tokyo Stock Exchange. 9613.T dropped — live-tested empty at 5y
// history (wrong/delisted symbol, not a real coverage gap).
const JAPAN_TOPIX100 = [
...
export const SEED_UNIVERSES: Record<string, string[]> = {
