...
export async function dailyBriefingTask(logId: number | null = null): Promise<void> {
  if (await skipIfDisabled("daily_briefing", logId)) return;
  await wrapJobExecution("daily_briefing", logId, runDailyBriefing);
}
