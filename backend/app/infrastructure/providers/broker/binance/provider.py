...
class BinanceClient:
    ...
    def get_spot_trade_candidates(self, assets: set[str]) -> List[dict[str, Any]]:
        """Best-effort trade history for currently-held assets: probes
        {asset}{quote} for each common quote pair, skipping non-existent pairs."""
        trades = []
        for asset in assets:
            for quote in SPOT_TRADE_QUOTES:
                if asset == quote:
                    continue
                trades.extend(self.get_spot_trades(f"{asset}{quote}"))
        return trades
