...
def evaluate_health_status(quote_age: int | None, news_age: int | None, signal_age: int | None) -> str:
    ...
    news_healthy = _evaluate_news_sla(news_age) if news_age is not None else True
    signal_healthy = _evaluate_signal_sla(signal_age) if signal_age is not None else True
...
class AssetHealthService(BaseService):
    def __init__(self, health_repo: AssetHealthRepository, market_repo: MarketRepository):
