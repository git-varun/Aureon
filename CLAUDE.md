...
Aureon is a personal portfolio management platform covering equities (Zerodha/Groww), crypto (Binance), and other asset  
 ...
 classes. It has an Express/TypeScript backend (`backend-node/`, Prisma ORM), React/Vite frontend, PostgreSQL database,  
   
 and Redis (cache + BullMQ job queue). The Python/FastAPI/Celery backend (`backend/`) that originally served this app  
   
 was fully ported to Node and deleted on 2026-08-16 — there is no Python anywhere in this repo any more.  
...
**Project layout**  
aureon/  
  ├── backend-node/     # Express/TypeScript API + BullMQ workers, tests  
  │   ├── src/  
...
**Commands**  
**Backend (run from backend-node/)**  
 cd backend-node  
 npm run dev       # API server (tsx watch src/index.ts), port 8010  
 npm run worker    # BullMQ worker (scripts/startWorker.ts) — job execution + repeatable schedules  
 ...
 npx vitest run src/routes/market/market.test.ts  
  
**Database migrations (run from backend-node/)**  
Prisma is the canonical source of schema truth (`backend-node/prisma/schema.prisma`).  
...
**Architecture**  
**Backend layout (**backend-node/src/**)**  
...
**AI service (**backend-node/src/lib/ai/**)**  
