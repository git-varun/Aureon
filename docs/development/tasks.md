...
Celery app: `app/workers/celery_app.py`. Tasks in `app/workers/`.
Broker: Redis. Result backend: Redis.
Startup validation checks require connection to Redis for the API and Celery workers to run successfully.
...
Configured via `task_routes` in `app/workers/celery_app.py`.

| Queue            | Worker container         | Concurrency | Time limit | Tasks                                                                                                            |
|------------------|--------------------------|-------------|------------|------------------------------------------------------------------------------------------------------------------|
| `q_ingestion`    | `celery_worker`          | 4           | default    | All tasks default to this queue (configured for `app.workers.ingestion.tasks.*`)                                 |
