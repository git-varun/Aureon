...
- [ ] **Step 1: Re-verify every precondition** against the live repo state — don't trust this plan's snapshot.
- [ ] **Step 2: Cut over the remaining Task-4 broker-sync routes in `vite.config.js`** per the scope decision above — flip `/api/v1/portfolio/sync`, `/api/v1/portfolio/sync/status`, `/api/v1/portfolio/portfolios/:id/sync/binance/backfill(/status)?`, and `/api/v1/config/providers/zerodha/oauth/callback` to `apiNodeProxyTarget`, removing the now-obsolete "stays on Python" guard comments and replacing them with a comment stating these were cut over unverified per the 2026-08-16 scope decision, pending the user's manual real-account testing. Run `tsc --noEmit` and the relevant `backend-node` test files for these routes (`sync.test.ts`, `providers.test.ts` or equivalent) to confirm nothing is structurally broken by the flip — this does not substitute for live credential verification, which remains the user's task.
- [ ] **Step 3: Rewrite `bootstrap.sh`** to run the Node-only equivalent of every step, confirming each Node job/script it calls actually exists by reading the actual file, not assuming from a task name.
- [ ] **Step 4: Update `docker-compose.yml`.**
- [ ] **Step 5: Update `CLAUDE.md`** — architecture sections only, scoped as described above.
- [ ] **Step 6: Delete `backend/`.**
- [ ] **Step 7: Run the full app from a clean checkout** (fresh clone or fresh worktree, fresh Docker volumes) to confirm `bootstrap.sh` and the full stack work with zero Python present — this is the single most important verification step in this entire plan, since it's the point of no return.
- [ ] **Step 8: Commit** — one large commit, only after Step 7 passes.
