...
```
portfolio_snapshot: portfolio_id(UUID,PRIMARY KEY), market_value(NUMERIC),
                    cash_balance(NUMERIC), allocation(JSONB), daily_return(NUMERIC),
                    total_return(NUMERIC), updated_at(TIMESTAMP)
```
...
providers:      id(UUID), name(VARCHAR,UNIQUE), priority(INT), is_enabled(bool),
                health_status(VARCHAR), last_success_at(TIMESTAMP)

provider_usage: provider_id(UUID,FK→providers), endpoint(VARCHAR), request_count(BIGINT),
                cost_estimate(NUMERIC), recorded_at(TIMESTAMP)
