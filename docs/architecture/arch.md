...
React SPA → FastAPI (single process) → PostgreSQL (primary only)
                                     → Redis (cache + Celery broker/backend, same node)
                                     → Celery workers (all tasks, one pool)
```

Key: `/api/state` (registered in `app/api/main.py` and handled by `app/api/compatibility.py`) is the frontend's only data source.
...
- Files: `app/workers/celery_app.py` (add `task_routes`), `docker-compose.yml` (3 worker services with `-Q` flags)
...
| Redis single node, persistent queues  | Celery broker and result backend running on single persistent Redis instance |
